...
There has been a great deal written about the concept of fitness, including multiple conflicting definitions, which ``\emph{is hardly surprising as every important scientific concept is difficult to understand from first principles, as for instance the notions of space and time, or energy and force}'' \parencite[p. 1358][]{Wagner2010}.
Thus, some conflicting aspects of the definitions of fitness are:
\begin{enumerate}
	\item \label{list:prop} Whether fitness is a realized value or a propensity.
	\item \label{list:level} Definitions vary depending on whether fitness is defined at the level of the genetic lineage \parencite[e.g.][]{Akc2016}, of the individual \parencite[e.g.][]{Cam2000}, of the genotype \parencite[e.g.][]{Steiner2012}, of the population\dots
	\item On what time frame should fitness be measured. It is the asymptotic number of descendants or the contribution to the next generation?
	\item What is being maximized by natural selection \parencite{Birch}
	\item 
	\item \label{list:info} Amount of information about the environment that populations accumulate by selection \parencite{Frank2012V}
\end{enumerate}
...
On the point \ref{list:prop}, it is now rather consensual that the concept of fitness is more useful when it is defined as a propensity, that is, as an expected value that cannot be measured directly because of stochasticity \parencite{Brandon1984,Price1996,Krimbas2004}. A propensity definition makes easier to consider fitness at the level of individuals (point \ref{list:level}), as will be further developed in chapter \ref{chap:dynhet}.
We will consider fitness as the contribution to the next generation only. Asymptotic definitions are difficult to apply in changing environments or in the presence of density dependence. 
 
most adapted to the study framework.
I see great conceptual promises in the point \ref{list:info}, about defining fitness in term of information, and it might enlighten some of the work carried out, but it did not influence the work directly and is not necessary to understand it.
