...
\begin{frame}
	...
	\begin{figure}
		\only<1>{\includegraphics[width=\textwidth]{Figures/figure/dice1-1.pdf}}
		\only<2>{\includegraphics[width=\textwidth]{Figures/figure/dice1-2.pdf}}
		\only<3>{\includegraphics[width=\textwidth]{Figures/figure/dice1-3.pdf}}
		\only<4>{\includegraphics[width=\textwidth]{Figures/figure/dice1-4.pdf}}
		\only<5>{\includegraphics[width=\textwidth]{Figures/figure/dice1-5.pdf}}
		\only<6>{\includegraphics[width=\textwidth]{Figures/figure/dice1-6.pdf}}
		\only<7>{\includegraphics[width=\textwidth]{Figures/figure/dice1-7.pdf}}
	\end{figure}
...
\node[roundblock,text width=3cm,minimum height=0.2cm] (test) at (cordtest) {\tikz{\node[anchor=north] (ts) at (0.825,0.6){\centering\textbf{\color{OwlYellow}{Individual differences?}}};
\node[roundblock,text width=2.6cm,minimum height=0.2cm] (MM) at (0,-0.1) {\parbox[t][0.2cm][t]{2.6cm}{\centering\textbf{Mixed models}}};
}};
}
%\node[roundblock,text width=2.6cm,minimum height=0.5cm] (MM) at (0,-0.1) {\parbox[t][1.3cm][t]{1.2cm}{\centering\textbf{Mixed models}}};\\ \node[roundblock,text width=2.6cm,minimum height=0.5cm,anchor=north] (NS) at (1.5,-0.1) {\parbox[t][1.3cm][t]{1.2cm}{\centering\textbf{Neutral matrices}}};
 %}};
...
\begin{tikzpicture}[%
  % common options for blocks:
  block/.style = {draw, align=center, anchor=north,
              minimum height=0.50cm, inner sep=1pt,fill=white},
	roundblock/.style = {draw, align=center, anchor=north,
              minimum height=2cm, inner sep=2pt,rounded corners=3pt,fill=white},
	backblock/.style = {fill=gray!10, align=center, anchor=west,
               inner sep=2pt,rounded corners=3pt},
  % common options for the circles:
  ball/.style = {circle, draw, align=center, anchor=north, inner sep=0},
	oval/.style = {ellipse, draw, align=center, anchor=north, inner sep=1pt,fill=white}, 
...
\node[backblock,minimum width=9.8cm,minimum height=4.2cm] (Abb) at (-4,-1) {};
\node[backblock,fill=gray!25,minimum width=7cm,minimum height=4.2cm] (Bbb) at (6,-1) {};

\node[backblock,fill=gray!40,minimum width=17cm,minimum height=1.8cm] (Cbb) at (-4,-4.2) {};

\node[roundblock,text width=3cm] (Parameters) at (-2,0) {\textbf{Simulation parameters}\\ \tikz{\node[roundblock,text width=1.3cm,minimum height=1cm] (svparam) at (0,0) {\parbox[t][1.3cm][t]{1.2cm}{ \centering $\bar{\rho}$, $\phi$ \\ \dots \vfill {\scriptsize Estimated}}}; \node[roundblock,text width=1.3cm,minimum height=1cm] (estparam) at (1.5,0) {\parbox[t][1.3cm][t]{1.2cm}{ \centering $\boldsymbol{\sigma_{\rho}^2}$, $\boldsymbol{\sigma_{\phi}^2}$, $m$, $c$ \vfill {\scriptsize Unknown}}}}};
	
\node[block,text width=2cm] (scenario1) at ($(Parameters.north)+(3,0)$) {Scenario 1};
\node[text width=2cm, align=center] (par) at ($(scenario1)+(0,0.8)$) {Parameter};
\node[text width=2cm, align=center] (comb) at ($(par)+(0,-0.3)$) {combinations};
\node[block,text width=2cm] (scenario2) at ($(scenario1)+(0,-0.6)$) {Scenario 2};
\node[] (scenarion) at ($(scenario1)+(0,-1.45)$) {\textbf{\vdots}};
\node[block,text width=2cm] (scenario240) at ($(scenario1)+(0,-2)$) {Scenario 249};

\coordinate (stos) at (3,-0.0275);

\node[oval,text width=2.3cm, inner sep=-1pt] (sv) at ($(Parameters.south)+(6,-1.2)$) {Snow vole data set};
\node[oval,text width=2.2cm] (simdata1) at ($(scenario1.north) + (stos)$) {1000 data sets};
\node[oval,text width=2.2cm] (simdata2) at ($(scenario2.north) + (stos)$) {1000 data sets};
\node[] (simn) at ($(scenarion)+(stos)$) {\textbf{\vdots}};
\node[oval,text width=2.2cm] (simdata240) at ($(scenario240.north) + (stos)$) {1000 data sets};

\gettikzxy{(sv.west)}{\svwx}{\svwy}
\gettikzxy{(Parameters.south)}{\psx}{\psy}
\draw[->,thick] (sv.west) -- (-2.7,\svwy) -- ($(-2.7,\psx)+(0,-0.4)$);
...
\gettikzxy{(Parameters.north)}{\px}{\py}
\coordinate (cordtest) at ($(\ax,\py)+(2.5,0)$);
\node[roundblock,text width=3cm,minimum height=0.2cm] (test) at (cordtest) {\tikz{\node[anchor=north] (ts) at (0.825,0.6){\textbf{\{$\boldsymbol{\hat{\sigma}_{\rho}^2}$,$\boldsymbol{\hat{\sigma}_{\phi}^2}$\}=\{0,0\} ? }};
\node[roundblock,text width=1.3cm,minimum height=1.5cm] (MM) at (0,-0.1) {\parbox[t][1.3cm][t]{1.2cm}{\centering\textbf{MM:} \vspace{0.1cm} \footnotesize \\ LRT$_\phi$ \\ LRT$_\rho$}}; \node[roundblock,text width=1.3cm,minimum height=1.5cm,anchor=north] (NS) at (1.5,-0.1) {\parbox[t][1.3cm][t]{1.2cm}{\centering\textbf{NS:}\footnotesize \\ KS, $\chi^2$,\\ M, V,\\ H, P}};
 }};
%
...
%%\draw [decorate, decoration = {brace, amplitude = 10pt}]  ($(simdata1.east)+(0.15,0.25)$) -- ($(simdata240.east)+(0.15,-0.25)$);
%
%\node[block,text width=2cm,anchor=west, inner sep=2pt] (sig) at ($(test.east)+(0.7,0)$) {Type I and II error rates};
%\draw[->,double,thick] (test.east) to (sig.west);
%
%\gettikzxy{(sv.east)}{\svx}{\svy}
%\gettikzxy{(test.south)}{\tsx}{\tsy}
%\gettikzxy{(sig.south)}{\ssx}{\ssy}
%\coordinate (cordinf) at ($(\ax,\svy)+(2,-0.2)$);

%\draw[->, dashed, thick] (sig.south) to [out=270,in=350] (cordinf);
%
%\draw[->,shorten >= 1pt, dashed, thick] (sig.south) -- ($(\ssx,\svy)+(0,0.35)$) -- ($(\tsx,\svy)+(0,0.35)$);
%
%\draw[->,shorten >= 2pt,thick] (sv.east) -- (\tsx,\svy) -- (test.south);


% indices for caption
\node[] (a) at ($(scenario1.north)+(-3,0.7)$){\textbf{A: Data simulations}};
\node[] (b) at ($(test.north)+(1.5,0.7)$) {\textbf{B: Tests for fixed heterogeneity}};
