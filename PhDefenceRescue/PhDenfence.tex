...
\end{figure}
\uncover<10->{
\begin{alertblock}{Results so far}
	\begin{enumerate}
		\uncover<10->{\item Apparent stasis\dots}
		\uncover<11->{\item \dots but genetic change towards lower mass}
		\uncover<12->{\item \textit{What is the selective pressure? Why don't we see it?}}
	\end{enumerate}
\end{alertblock}
} 
\end{frame}
%%%%%%%%%%%
\subsection[Selective mechanism]{Selective mechanism}

\begin{frame}
\frametitle{Snow falls and ontogeny}
\vspace{-15pt}
	\begin{figure}
		\includegraphics[width=0.5\textwidth]{Figures/BDsnow-1}
		\uncover<2->{\includegraphics[width=0.5\textwidth]{Figures/Climatic_trend-1}}
	\end{figure}
\vspace{-15pt}	
	\begin{alertblock}{}
		\begin{itemize}
			\uncover<1->{\item Snow falls constrain birth dates}
			\uncover<2->{\item Snow-free season has shortened}
			\uncover<3->{\item \textit{Less time to grow $\rightarrow$ Selection for smaller voles?}}
		\end{itemize}
	\end{alertblock}
\end{frame}
%%%%%%%%%%%
...
\begin{frame}
	\begin{figure}
		\includegraphics[width=\textwidth]{Figures/concetpualplot-1}
	\end{figure}
\end{frame}
%%%%%%%%%%%
